        ...
        for user in participants:
            ...
            if self.config['optimizer'] == 'sgd':
                optimizers = torch.optim.SGD([
                    {"params": model_client.affine_output.parameters(), "lr": self.config['lr'],
                     "weight_decay": self.config['l2_regularization']},
                    {"params": model_client.embedding_item.parameters(), "lr": self.config['lr'],
                     "weight_decay": self.config['l2_regularization']}
                ])
            ...
            for key in self.client_model_params[user].keys():
                ...
                # self.client_model_params[user][key] = self.client_model_params[user][key].data
