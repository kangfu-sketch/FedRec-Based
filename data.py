...
class UserItemRatingDataset(Dataset):
    ...
    def __init__(self, user_tensor, item_tensor, target_tensor, time_tensor=None):
        """
        args:
    ...
    def __getitem__(self, index):
        try:
            return self.user_tensor[index], self.item_tensor[index], self.target_tensor[index], self.time_tensor[index]
        except:
...
class SampleGenerator(object):
    ...
    def _split_loo(self, ratings):
        ...
        return train[['userId', 'itemId', 'rating', 'timestamp']], val[['userId', 'itemId', 'rating', 'timestamp']], test[['userId', 'itemId', 'rating', 'timestamp']]

    def _sample_negative(self, ratings):
        """return all negative items & 100 sampled negative items"""
        interact_status = ratings.groupby('userId')['itemId'].apply(set).reset_index().rename(
            columns={'itemId': 'interacted_items'})
        interact_status['negative_items'] = interact_status['interacted_items'].apply(lambda x: self.item_pool)
    ...
    def store_all_train_data(self, num_negatives):
        ...
        for userId, user_train_ratings in grouped_train_ratings:
            ...
            for row in user_train_ratings.itertuples():
                ...
                inter_timestamp.append(row.timestamp)
            ...
            timestamp.append(inter_timestamp)
            single_user = []
            user_item = []
            user_rating = []
            inter_timestamp = []
        ...
        return [users, items, ratings, timestamp]
