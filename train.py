...
parser.add_argument("--pre_embedding", action='store_true', default=False)
...
if config['dataset'] == "ml-100k":
    ...
    print("read dataset error.")
    exit()

config['num_users'] = len(set(rating['uid']))
config['num_items'] = len(set(rating['mid']))

# Load pre-trained embeddings
if config['pre_embedding']:
    embedding_file = f'./data/{config["dataset"]}/pre_embeddings_{config["latent_dim"]}.npy'
    pre_trained_embeddings = np.load(embedding_file)
    pre_trained_embeddings = torch.tensor(pre_trained_embeddings, dtype=torch.float32)
else:
    pre_trained_embeddings = None

engine = MLPEngine(config, pre_trained_embeddings)
...
for round in range(config['num_round']):
    ...
    tr_loss = engine.fed_train_a_round(all_train_data, round)
...
logging.info('clients_sample_ratio: {}, bz: {}, lr: {}, dataset: {}, factor: {}, negatives: {}'.
             format(config['clients_sample_ratio'], config['batch_size'], config['lr'],
